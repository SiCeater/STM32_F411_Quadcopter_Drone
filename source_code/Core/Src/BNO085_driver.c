...
int BNO080_Initialization(void)
{
	...
	// if (debug)
	// {
	// 	sprintf(buffer_debug,"\nBNO080 Not OK: 0x%02x Should be 0x%02x\n", shtpData[0], SHTP_REPORT_PRODUCT_ID_RESPONSE);
	// 	print_to_console(buffer_debug,strlen(buffer_debug));
	// }
...
int BNO080_readFRSdata(uint16_t recordID, uint8_t startLocation, uint8_t wordsToRead)
{
	...
	while (1)
	{
		...
		if (spot >= MAX_METADATA_SIZE)
		{
			...
			//printf("metaData array over run. Returning."); // a modifier
...
int BNO080_receivePacket(void)
{
	...
	uint8_t packetLSB = SPI1_SendByte(0);
	uint8_t packetMSB = SPI1_SendByte(0);
	uint8_t channelNumber = SPI1_SendByte(0);
	uint8_t sequenceNumber = SPI1_SendByte(0); //Not sure if we need to store this or not
...
int BNO080_sendPacket(uint8_t channelNumber, uint8_t dataLength)
{
	...
	SPI1_SendByte(packetLength & 0xFF);					//Packet length LSB
	SPI1_SendByte(packetLength >> 8);					//Packet length MSB
	SPI1_SendByte(channelNumber);						//Channel number
	SPI1_SendByte(sequenceNumber[channelNumber]++); 	//Send the sequence number, increments with each packet sent, different counter for each channel

	//Send the user's data packet
	for (uint8_t i = 0; i < dataLength; i++)
	{
		SPI1_SendByte(shtpData[i]);
	}
...
void imu_init()
{
	...
	//Enable Magnetic Field output
	BNO080_calibrateAll(); //Turn on cal for Accel, Gyro, and Mag
	BNO080_enableRotationVector(1000); //Send data update every ms (1000Hz)
}
